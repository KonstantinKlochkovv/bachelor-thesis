...
population = 1e5
max_sizes = np.arange(population//2, population, 1e4)
...
A4_WIDTH = 8.27  # Ширина A4
A4_HEIGHT = 11.69/2.5  # Высота A4 (можно уменьшить, если нужно)
fig, ax = plt.subplots(2, 5, figsize=(A4_WIDTH, A4_HEIGHT), sharey=True, sharex=True)
...
for i in range(2):
    for k, caption in enumerate(captions):
        for big_size in max_sizes:
            ...
            ax[i][k].errorbar(x=big_size//1e4, y=indices[index][i], yerr=indices[f'{index}_conf'][i], color=colors[0], capsize=5, fmt='.')

            indices = sobol.analyze(problem, data[:,k+len(captions)], calc_second_order=False)
            ax[i][k].errorbar(x=big_size//1e4, y=indices[index][i], yerr=indices[f'{index}_conf'][i], color=colors[1], capsize=5, fmt='.')
